...
function emptyRow() {
  return {
    ...
    meta: { salary: "", location: "", mode: "", field: "", skills: [] },
  };
}

function normalizeMeta(m) {
  if (!m || typeof m !== "object") return { salary: "", location: "", mode: "", field: "", skills: [] };
  return {
    salary: typeof m.salary === "string" ? m.salary : "",
...
function updateRow(id, patch) {
  ...
  if (patch.meta && typeof patch.meta === "object") {
    if (!row.meta) row.meta = normalizeMeta(null);
    const m = patch.meta;
    if (typeof m.salary === "string") row.meta.salary = m.salary;
    if (typeof m.location === "string") row.meta.location = m.location;
    if (typeof m.mode === "string") row.meta.mode = m.mode;
    if (typeof m.field === "string") row.meta.field = m.field;
    if (Array.isArray(m.skills)) row.meta.skills = m.skills.filter((s) => typeof s === "string");
  }
